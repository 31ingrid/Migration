...
pops=function(Linf,K,A0,psi,theta,fishsel1,fishsel2,natmort,Q1,Q2,SS0,m,MIG,sigmaR2_1,sigmaR2_2,Nyrs)
{
 ...
 for (y in 2:Nyrs){
   ...
   if (MIG==1){
     Natage_P1_Nyrs[y,]=VBRec_P1*(1-m) #m is migration rate from 1 to 2
     Natage_P2_Nyrs[y,]=VBRec_P2+(VBRec_P1*m)
   ...
   if (MIG==2){
     Natage_P1_Nyrs[y,]=VBRec_P2*m+VBRec_P1*(1-m);
     Natage_P2_Nyrs[y,]=VBRec_P1*m+VBRec_P2*(1-m);
...
for(exp in 1:1){#was 1:11
 ...
 for(popstat in 1:8){  #was 1:4
  ...
  setwd("/Users/ingrid.spies/Documents/GOA_cod/SimulationResults/COV_2pops_GM_age0migration/")
