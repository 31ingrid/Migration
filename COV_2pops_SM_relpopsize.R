...
library(matrixStats)
#Parameters based on Blackspotted RF
L_all=vector()
Wt_all=vector()
fishsel=vector()
srvsel=vector()
...
FINMATmeanSM=matrix(0,12,11);FINMATvarSM=matrix(0,12,11);FINMATvarSM_raw=matrix(0,12,11);FINMATmeanSM_raw=matrix(0,12,11);
for(exp in 1:1){#was 1:11
 ...
 fishsel2=fishsel
 natmort=c(0.05,0.05)
# SS0=c(1,1) #initial stock biomass.
 Q1=mat
 Q2=mat_a5012
 ...
 if (exp==1){natmort=c(.05,.05);Q1=mat;Q2=mat;sigmaR2_1=0.64;sigmaR2_2=0.64}
 ...
 if (exp==6){Q2=mat}
 if (exp==7){Q1=mat}
 if (exp==8){sigmaR2_1=0.25;sigmaR2_2=0.85}
 ...
 for(popstat in 1:12){  
  ...
  if(popstat==2){SS0=c(4340,7500)}
  if(popstat==3){SS0=c(43,75)}
  if(popstat==4){SS0=c(43400,75000)}
  if(popstat==5){SS0=c(750,434)}
  if(popstat==6){SS0=c(7500,4340)}
  if(popstat==7){SS0=c(75,43)}
  if(popstat==8){SS0=c(75000,43400)}
  if(popstat>8){MIG=2;}
  if(popstat==9){SS0=c(434,750)}
  if(popstat==10){SS0=c(4340,7500)}
  if(popstat==11){SS0=c(43,75)}
  if(popstat==12){SS0=c(43400,75000)}
  ...
  setwd("/Users/ingrid.spies/Documents/GOA_cod/SimulationResults/COV_2pops_SM_relpopsize/")#in 5 I increased the sigmaS to .4 and also added another one way migration to the other way.
  ...
  for (k in 1:Nrep){  #k is number of Nrep
   for(j in 1:length(m)){   #m is the different migration rates you are testing
    ...
    cor2=sort(cor);#1 means that there was some negative correlation.
