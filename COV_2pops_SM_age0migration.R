...
pops=function(Linf,K,A0,psi,theta,fishsel1,fishsel2,natmort,Q1,Q2,SS0,m,MIG,sigmaR2_1,sigmaR2_2,Nyrs)
{
 ...
 migrants_to2=vector();migrants_to1=vector();migrants=vector()
 rate=vector()
 ...
 for (y in 2:Nyrs){
   ...
   if (MIG==1){
     ...
     migrants_to2[y]=Natage_P1_Nyrs[y,1]*m/sum(Natage_P2_Nyrs[y,])#how many is 2 getting relative to what it had
   ...
   if (MIG==2){
     ...
     migrants_to2[y]=Natage_P1_Nyrs[y,1]*m/sum(Natage_P2_Nyrs[y,])#how many is 2 getting relative to what it had. denominator is already adjusted
     migrants_to1[y]=Natage_P2_Nyrs[y,1]*m/sum(Natage_P1_Nyrs[y,])#how many is 1 getting relative to what it had
   } #number in P2 that move to P1 plus the ones that do not leave P1.
   
   
 }#repeat number of runs
 
 #Is there a lag before they become correlated due to the long life history?
# plot(rowSums(Natage_P1_Nyrs[245:295,]),type="l")
# lines(rowSums(Natage_P2_Nyrs[247:297,]),col="blue",lwd=2)
 ...
 #pcor=cor.test(rowSums(P1_srvsel[49:(Nyrs-2),]),rowSums(P2_srvsel[51:Nyrs,]),alternative="greater")
...
for(exp in 1:1){#was 1:11
 ...
 for(popstat in 1:8){  #was 1:4
  if(popstat<5){ MIG=1;}
  if(popstat==1){SS0=c(750,750)}
  if(popstat==2){SS0=c(430,750)}
  if(popstat==3){SS0=c(75,750)}
  if(popstat==4){SS0=c(750,75)}
  if(popstat>4){ MIG=2;}
  if(popstat==5){SS0=c(750,750)}
  if(popstat==6){SS0=c(430,750)}
  if(popstat==7){SS0=c(75,750)}
  if(popstat==8){SS0=c(750,75)}
  ...
  setwd("/Users/ingrid.spies/Documents/GOA_cod/SimulationResults/COV_2pops_SM_age0migration/")
...
ggplot(Res)+geom_line(aes(x=Year,y=Number,col=Name))+theme_bw(base_size=14)
